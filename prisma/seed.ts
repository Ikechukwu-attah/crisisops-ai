...
async function main() {
  console.log("Seeding IncidentMemory...");
  await prisma.incidentMemory.deleteMany();
  const result = await prisma.incidentMemory.createMany({ data: SEED_MEMORIES });
  console.log(`Seeded ${result.count} memory records.`);
}
